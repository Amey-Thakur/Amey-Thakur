...
def main():
    ...
    try:
        ...
        # IMPACT ANALYSIS
        # The distinct owners of every repository a pull request was sent to.
        #
        # Search returns 30 items a page. Reading only the first response
        # sampled 30 of several hundred pull requests, so this count was right
        # only while those 30 happened to touch every owner, and would have
        # fallen silently once recent work clustered in one place. Paginating
        # at the maximum page size makes it right by construction. Search stops
        # at 1000 results, which the loop respects and the account is far below.
        SEARCH = f"https://api.github.com/search/issues?q=author:{username}+type:pr&per_page=100"
        ...
        if pr_search:
            ...
            page = 1
            while pr_search:
                items = pr_search.get('items', [])
                for item in items:
                    repo_url = item.get('repository_url', '')
                    match = re.search(r'/repos/([^/]+)/', repo_url)
                    if match: unique_contexts.add(match.group(1))
                if len(items) < 100 or page * 100 >= reachable:
                    break
                page += 1
                pr_search = fetch_data(f"{SEARCH}&page={page}", token)
